...
export function AdminDashboard() {
  ...
  useEffect(() => {
    api.get('/users/dashboard/stats').then(({ data }) => setStats(data))
      .catch(() => {})
      .finally(() => setLoading(false))
  }, [])

  const kpis = [
    { label: 'Total Usuarios', value: stats?.usuarios.total ?? '—', icon: Users, color: 'text-primary-400', bg: 'bg-primary-500/10' },
    { label: 'Técnicos Activos', value: stats?.usuarios.tecnicos ?? '—', icon: Shield, color: 'text-violet-400', bg: 'bg-violet-500/10' },
    { label: 'Sol. Aprobadas', value: stats?.solicitudes['APROBADO'] ?? '—', icon: CheckCircle2, color: 'text-success-400', bg: 'bg-success-500/10' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="glass-card p-6">
          ...
          {[
            ...
            { key: 'EN_REVISION', label: 'Revisión Técnica', color: 'bg-yellow-500' },
