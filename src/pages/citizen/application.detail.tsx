  ...
  return (
    <div className="animate-fade-in space-y-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-start gap-4">
        <Link to="/citizen/applications" className="btn-secondary p-2 mt-1">
          <ArrowLeft size={18} />
        </Link>
      ...
      <div className="glass-card p-6 pb-12 mb-6">
        <h2 className="font-heading font-semibold text-blue-955 mb-6 text-sm">Progreso del Trámite</h2>
      ...
      {(application.status === 'PENDIENTE_PAGO' || application.status === 'PAGADO' || application.status === 'APROBADO') && application.payments?.length > 0 && (
        <div className="glass-card p-6" style={{ borderColor: application.payments[0].status === 'PAGADO' ? 'rgba(22,163,74,0.3)' : 'rgba(124,58,237,0.3)' }}>
      ...
      {application.architect && (
        <div className="glass-card p-6">
          ...
          <p className="text-blue-955 font-medium">{application.architect.first_name} {application.architect.last_name}</p>
          <p className="text-slate-500 text-sm">{application.architect.email}</p>
        </div>
      )}
