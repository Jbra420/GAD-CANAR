...
export function MyApplications() {
  ...
  const fetchMyApplications = async () => {
    try {
      set_is_loading(true)
      const { data } = await applications_api.myApplications()
              ...
              {is_loading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center">
                    ...
                    <p className="text-blue-955 font-medium">Aún no tienes solicitudes registradas</p>
                ...
                applications.map((s) => (
                  <tr key={s.id} className="border-b border-surface-border hover:bg-surface-muted/50 transition-colors">
                    <td className="px-6 py-4">
                      <p className="font-medium text-blue-955">{s.procedure_type || 'Ordenamiento Territorial'}</p>
