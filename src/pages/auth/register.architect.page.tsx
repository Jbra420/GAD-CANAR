...
const registerArchitectSchema = z.object({
  first_name: z.string().min(2, 'Ingresa tu nombre'),
  last_name: z.string().min(2, 'Ingresa tu apellido'),
  national_id: z.string().regex(/^\d{10}$/, 'La cédula debe tener 10 dígitos'),
...
type RegisterArchitectForm = z.infer<typeof registerArchitectSchema>

export function RegisterArchitectPage() {
  const [show_pass, set_show_pass] = useState(false)
  const [is_loading, set_is_loading] = useState(false)
  const [error, set_error] = useState<string | null>(null)
  const [is_success, set_is_success] = useState(false)

  const { register, handleSubmit, formState: { errors } } = useForm<RegisterArchitectForm>({
    resolver: zodResolver(registerArchitectSchema),
  })

  const onSubmit = async (data: RegisterArchitectForm) => {
    set_is_loading(true)
    set_error(null)
    try {
      ...
      set_error(e.response?.data?.message || 'Error al registrar. Intenta nuevamente.')
    } finally {
      ...
      <div className="relative z-10 w-full max-w-lg space-y-6">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="rounded-3xl p-6 space-y-5 bg-white border border-slate-200 shadow-xl">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input {...register('first_name')} id="reg-nombre" className="input-field pl-9"
                      placeholder="Juan" />
                  </div>
                  {errors.first_name && <p className="text-red-500 text-xs mt-1 flex items-center gap-1"><AlertCircle size={11} />{errors.first_name.message}</p>}
                </div>
                <div>
                  <label className="input-label text-slate-600">Apellido *</label>
                  <input {...register('last_name')} id="reg-apellido" className="input-field" placeholder="Pérez" />
                  {errors.last_name && <p className="text-red-500 text-xs mt-1 flex items-center gap-1"><AlertCircle size={11} />{errors.last_name.message}</p>}
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input {...register('national_id')} id="reg-cedula" className="input-field" placeholder="0102030405" maxLength={10} />
                {errors.national_id && <p className="text-red-500 text-xs mt-1 flex items-center gap-1"><AlertCircle size={11} />{errors.national_id.message}</p>}
              </div>
              <div>
                <label className="input-label text-slate-600">Teléfono</label>
                <div className="relative">
                  <Phone size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                  <input {...register('phone')} id="reg-telefono" className="input-field pl-9" placeholder="0987654321" />
                </div>
              </div>
            </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input {...register('password')} id="reg-password" type={show_pass ? 'text' : 'password'}
                    ...
                    <button type="button" onClick={() => set_show_pass(!show_pass)}
                      ...
                      {show_pass ? <EyeOff size={14} /> : <Eye size={14} />}
                    </button>
                  </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <Lock size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                    <input {...register('confirm_password')} id="reg-confirm-password" type="password"
          ...
          <button
            ...
            style={{ opacity: is_loading ? 0.8 : 1 }}
          >
            {is_loading
