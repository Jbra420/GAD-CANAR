  ...
  return (
    <div className="animate-fade-in space-y-6 max-w-3xl mx-auto">
      ...
      <div className="glass-card p-6 pb-12 mb-6">
        <h2 className="font-heading font-semibold text-blue-955 mb-6 text-sm text-left">Progreso del Trámite</h2>
      ...
      {(application.status === 'PENDIENTE_PAGO' || application.status === 'PAGADO' || application.status === 'APROBADO') && application.payments?.length > 0 && (
        <div className="glass-card p-6 text-left" style={{ borderColor: application.payments[0].status === 'PAGADO' ? 'rgba(22,163,74,0.3)' : 'rgba(124,58,237,0.3)' }}>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-heading font-semibold text-blue-955 flex items-center gap-2">
              <DollarSign size={18} style={{ color: application.payments[0].status === 'PAGADO' ? '#16A34A' : '#7C3AED' }} />
              Información de Pago
            </h2>
            <span className={cn(
              'px-3 py-1 rounded-full text-xs font-bold',
              application.payments[0].status === 'PAGADO' ? 'bg-green-100 text-green-700' : 'bg-purple-100 text-purple-700'
            )}>
              {application.payments[0].status === 'PAGADO' ? '✅ Pago Recibido' : '⏳ Pendiente de Pago'}
            </span>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-xs text-slate-500 font-bold uppercase mb-1">Monto a pagar</p>
              <p className="text-2xl font-bold text-blue-955">${Number(application.payments[0].amount).toFixed(2)}</p>
            </div>
            <div>
              <p className="text-xs text-slate-500 font-bold uppercase mb-1">Concepto</p>
              <p className="text-sm font-medium text-blue-955">{application.payments[0].concept}</p>
            </div>
          </div>
          {application.payments[0].status === 'PENDIENTE' && (
        ...
        <div className="glass-card p-6 text-left">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <p className="text-blue-955 font-medium mt-0.5">{application.citizen.first_name} {application.citizen.last_name}</p>
            </div>
            <div>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {is_draft && (
            <label className="btn-secondary text-xs px-3 py-2 cursor-pointer">
              <input type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={handleUploadAttachment} className="sr-only" />
              {is_uploading_file ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><Upload size={14} /> Agregar Documento</>}
            </label>
          )}
        </div>
      ...
      {is_draft && (
        <div className="glass-card p-6 border-primary/30 text-left">
          ...
          >
            {is_submitting ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><Send size={18} /> Enviar a Revisión</>}
          </button>
        </div>
      )}
    </div>
