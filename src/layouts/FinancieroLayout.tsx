...
import { LayoutDashboard, CreditCard, History, CheckSquare, LogOut, ChevronDown, Menu, X, Landmark } from 'lucide-react'
...
export function FinancieroLayout() {
  ...
  const initials = `${user?.nombre?.[0] ?? ''}${user?.apellido?.[0] ?? ''}` || 'F'

  return (
    <div className="min-h-screen flex flex-col bg-slate-50">
      <header className="sticky top-0 z-50 px-4 md:px-8 h-16 flex items-center justify-between"
        style={{ background: 'rgba(12,6,22,0.92)', backdropFilter: 'blur(24px)', borderBottom: `1px solid ${ACCENT}20`, boxShadow: '0 4px 30px rgba(0,0,0,0.3)' }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              ...
              <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.6rem', fontWeight: 600, letterSpacing: '0.1em' }}>PORTAL FINANCIERO</p>
            </div>
          </div>
        </div>
        <nav className="hidden xl:flex items-center gap-1 p-1 rounded-full" style={{ background: `${ACCENT}10`, border: `1px solid ${ACCENT}20` }}>
          {navItems.map(item => (
            <NavLink key={item.to} to={item.to} end={item.end}
              ...
              style={({ isActive }) => isActive ? { background: `linear-gradient(135deg, ${ACCENT} 0%, #5B21B6 100%)`, boxShadow: `0 0 20px ${ACCENT}50` } : {}}>
        ...
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full border" style={{ background: `${ACCENT}08`, borderColor: `${ACCENT}25` }}>
            ...
            <span style={{ color: `${ACCENT}cc`, fontSize: '0.7rem', fontWeight: 600 }}>Finanzas</span>
          </div>
          <div className="relative" ref={dropdownRef}>
            <button onClick={() => setDropdownOpen(!dropdownOpen)} className="flex items-center gap-2.5 p-1.5 pr-3 rounded-full transition-all border"
              ...
              <div className="w-7 h-7 rounded-full flex items-center justify-center font-bold text-xs" style={{ background: `linear-gradient(135deg, ${ACCENT} 0%, #5B21B6 100%)`, color: 'white' }}>{initials}</div>
              <span className="text-white text-sm font-medium hidden sm:block">{user?.nombre?.split(' ')[0] || 'Financiero'}</span>
            ...
            {dropdownOpen && (
              <div className="absolute right-0 mt-3 w-60 rounded-2xl overflow-hidden animate-slide-up"
                style={{ background: 'rgba(12,6,22,0.98)', backdropFilter: 'blur(20px)', border: `1px solid ${ACCENT}25`, boxShadow: `0 10px 40px rgba(0,0,0,0.5)` }}>
      ...
      {mobileMenuOpen && (
        <div className="xl:hidden fixed inset-0 z-40 pt-16 px-4 pb-6 flex flex-col" style={{ background: 'rgba(12,6,22,0.98)', backdropFilter: 'blur(20px)' }}>
