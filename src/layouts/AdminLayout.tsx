...
import { LayoutDashboard, Users, FileText, Activity, LogOut, ChevronDown, Menu, X, Shield } from 'lucide-react'
...
export function AdminLayout() {
  ...
  const initials = `${user?.nombre?.[0] ?? ''}${user?.apellido?.[0] ?? ''}` || 'A'

  return (
    <div className="min-h-screen flex flex-col bg-slate-50">
      <header className="sticky top-0 z-50 px-4 md:px-8 h-16 flex items-center justify-between"
        style={{ background: 'rgba(16,8,8,0.92)', backdropFilter: 'blur(24px)', borderBottom: `1px solid ${ACCENT}20`, boxShadow: '0 4px 30px rgba(0,0,0,0.3)' }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              ...
              <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.6rem', fontWeight: 600, letterSpacing: '0.1em' }}>ADMINISTRACIÓN</p>
            </div>
          </div>
        </div>
        <nav className="hidden xl:flex items-center gap-1 p-1 rounded-full" style={{ background: `${ACCENT}10`, border: `1px solid ${ACCENT}20` }}>
          {navItems.map(item => (
            <NavLink key={item.to} to={item.to} end={item.end}
              ...
              style={({ isActive }) => isActive ? { background: `linear-gradient(135deg, ${ACCENT} 0%, #9A181E 100%)`, boxShadow: `0 0 20px ${ACCENT}50` } : {}}>
        ...
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full border" style={{ background: `${ACCENT}08`, borderColor: `${ACCENT}25` }}>
            ...
            <span style={{ color: `${ACCENT}cc`, fontSize: '0.7rem', fontWeight: 600 }}>SuperAdmin</span>
          </div>
          <div className="relative" ref={dropdownRef}>
            <button onClick={() => setDropdownOpen(!dropdownOpen)} className="flex items-center gap-2.5 p-1.5 pr-3 rounded-full transition-all border"
              ...
              <div className="w-7 h-7 rounded-full flex items-center justify-center font-bold text-xs" style={{ background: `linear-gradient(135deg, ${ACCENT} 0%, #9A181E 100%)`, color: 'white' }}>{initials}</div>
              <span className="text-white text-sm font-medium hidden sm:block">{user?.nombre?.split(' ')[0] || 'Admin'}</span>
            ...
            {dropdownOpen && (
              <div className="absolute right-0 mt-3 w-60 rounded-2xl overflow-hidden animate-slide-up"
                style={{ background: 'rgba(16,8,8,0.98)', backdropFilter: 'blur(20px)', border: `1px solid ${ACCENT}25`, boxShadow: `0 10px 40px rgba(0,0,0,0.5)` }}>
                <div className="p-4" style={{ borderBottom: `1px solid ${ACCENT}15` }}>
                  ...
                  <div className="flex items-center gap-1.5 mt-3 px-2 py-1.5 rounded-lg w-max" style={{ background: `${ACCENT}15` }}>
                    ...
                    <span style={{ color: ACCENT, fontSize: '0.6rem', fontWeight: 700, textTransform: 'uppercase' }}>Super Administrador</span>
                  </div>
                </div>
                <div className="p-2">
          ...
          <button className="xl:hidden p-2 rounded-xl" style={{ color: 'rgba(255,255,255,0.8)', background: 'rgba(255,255,255,0.08)' }} onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
            {mobileMenuOpen ? <X size={19} /> : <Menu size={19} />}
          </button>
        </div>
      </header>
      {mobileMenuOpen && (
        <div className="xl:hidden fixed inset-0 z-40 pt-16 px-4 pb-6 flex flex-col" style={{ background: 'rgba(16,8,8,0.98)', backdropFilter: 'blur(20px)' }}>
      ...
      <div className="fixed top-0 left-0 w-full h-full overflow-hidden pointer-events-none z-0">
        ...
        <div className="absolute bottom-[-20%] left-[-10%] w-[50%] h-[50%] rounded-full opacity-[0.015] blur-[100px]" style={{ background: ACCENT }} />
      </div>
    </div>
  )
}
